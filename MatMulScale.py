	...
	for i in range(num_of_bram):
		for j in range(num_of_bram):
			if(i == 0 and j == 0):
				...
				file.write( '      c_reg_{a} <= c_reg_{b} | data_from_out_mat_{c}_{d};\n'
						.format(a = i*num_of_bram + j, b = i*num_of_bram + j - 1, c = i, d = j))
	file.write(			'      data_from_out_mat <= c_reg_{0};\n'.format(num_of_bram*num_of_bram - 1))
	file.write(			'    end\n'
						'  end\n\n')
	...
	for i in range(num_of_bram):
		for j in range(num_of_bram):
			...
			file.write(	'  .c_data(c_data_{a}_{b}),\n'
						'  .a_data_out(a_data_{a}_{b}_to_{a}_{c}),\n'
