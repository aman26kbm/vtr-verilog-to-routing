...
wire [`AWIDTH-1:0] bram_addr_a_for_writing;
...
assign bram_we_a = {`DESIGN_SIZE{bram_a_wdata_available}};
assign bram_wdata_a = matmul_c_data_out;

assign bram_addr_a_for_writing = bram_addr_c_NC;
assign bram_a_wdata_available = matmul_c_data_available;
...
assign start_mat_mul = start_tpu;
assign done_tpu = done_mat_mul;
...
`endif
  ...
  .a_data(bram_rdata_a),
  .b_data(bram_rdata_b),
  .a_data_in({`DESIGN_SIZE*`DWIDTH{1'b0}}),
  .b_data_in({`DESIGN_SIZE*`DWIDTH{1'b0}}),
