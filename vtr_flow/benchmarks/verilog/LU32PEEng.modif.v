...
wire [31:0] out1;
wire cout1;
adder_32bit u_add1(.a({8'b0, mem_addr}), .b({12'b0, Ndivk }), .cin(1'b0), .sumout(out1), .cout(cout1));
...
begin
	...
	begin
		ram_addr <= ram_addr + `BLOCKMDIVK; //ignore
		//mem_addr <= mem_addr + Ndivk;	//24 bit + 20 bit
		mem_addr <= out1 [23:0] ;
		mem_count <= mem_count + 2'b01; //ignore
	end
...
begin
	...
	begin
		if (mcount < comp_N)
			...
			next_state <= `MODE2_SETUP;
		else if (comp_N <= `BLOCKN + `BLOCKN) //ignore
			next_state <= `STALL;
		else
...
wire [31:0] out2;
wire cout2;
adder_32bit u_add2(.a({12'b0, N}), .b({24'b0, `BLOCKM}), .cin(1'b1), .sumout(out2), .cout(cout2));
wire [19:0] out2_1;
assign out2_1 = out2[19:0];


wire [31:0] out3;
wire cout3;
adder_32bit u_add3(.a({12'b0, comp_N}), .b({24'b0, `BLOCKN}), .cin(1'b0), .sumout(out3), .cout(cout3));

wire [31:0] out4;
wire cout4;
adder_32bit u_add4(.a({8'b0, mem_base}), .b({12'b0, mem_N}), .cin(1'b0), .sumout(out4), .cout(cout4));

wire [31:0] out5;
wire cout5;
adder_32bit u_add5(.a({25'b0, `MEMBLOCKN}), .b({8'b0, out4[23:0]}), .cin(1'b0), .sumout(out5), .cout(cout5));

wire [31:0] out6;
wire cout6;
adder_32bit u_add6(.a({8'b0, mem_cur}), .b({25'b0, `MEMBLOCKM}), .cin(1'b0), .sumout(out6), .cout(cout6));

wire [31:0] out7;
wire cout7;
adder_32bit u_add7(.a({8'b0, mem_left}), .b({25'b0, `MEMBLOCKM}), .cin(1'b0), .sumout(out7), .cout(cout7));

wire [31:0] out8;
wire cout8;
adder_32bit u_add8(.a({8'b0, mem_top}), .b({25'b0, mem_N}), .cin(1'b0), .sumout(out8), .cout(cout8));
...
begin
	...
	//Ndivk <= ((N+`BLOCKM-1)>>6)<<5; //20 bit + 8bit - 1bit
	Ndivk <= (out2_1>>6)<<5;
	mem_N <= Ndivk<<6;
	...
	begin
		/* mem_base <= mem_base + mem_N+`MEMBLOCKN; //24 bit + 20 bit + 7 bit
		mem_top <= mem_base + mem_N+`MEMBLOCKN; //24 bit + 20 bit + 7 bit
		mem_cur <= mem_base + mem_N+`MEMBLOCKN; //24 bit + 20 bit + 7 bit
		mem_left <= mem_base + mem_N+`MEMBLOCKN; //24 bit + 20 bit + 7 bit */
	...
	begin
		//mem_cur <= mem_cur + `MEMBLOCKM; // 24 bit + 7 bit
		mem_cur <= out6[23:0];
	end	
	else if (cur_state == `MODE3_SETUP)
	begin
		//mem_cur <= mem_cur + `MEMBLOCKM; // 24 bit + 7 bit
		//mem_left <= mem_left + `MEMBLOCKM; // 24 bit + 7 bit
		mem_cur <= out6[23:0];
		mem_left <= out7[23:0];
	end
	else if (cur_state == `MODE2_SETUP)
	begin
		//mem_cur <= mem_top + mem_N; // 24 bit + 7 bit
		//mem_top <= mem_top + mem_N; // 24 bit + 7 bit
		mem_cur <= out8[23:0];
		mem_top <= out8[23:0];
		mem_left <= mem_base;
	end
	...
	if (cur_state == `MODE0_SETUP || cur_state == `MODE2_SETUP || start) begin
		...
		mcount <= mcount+`BLOCKM; //ignore
	end

	if (cur_state == `MODE0_SETUP || start) begin
		ncount <= `BLOCKN;
	end else if (cur_state == `MODE2_SETUP) begin
		ncount <= ncount+`BLOCKN; //ignore
	end

	if (mcount < comp_N) begin
		block_m <= `BLOCKM;
	end else begin
		block_m <= comp_N - mcount + `BLOCKM; //ignore
	end 

	if (ncount < comp_N) begin
		block_n <= `BLOCKN;
	end else begin
		block_n <= comp_N - ncount + `BLOCKN; //ignore
	end
...
begin
	...
	start <= startDelay[15];
	mdivk <= (m+32-1)>>5; //ignore
end

//nextTopIdx + n + 1
	wire [17:0] out9;
	wire cout9;
	adder_18bit u_add9(.a({6'b0, nextTopIdx}), .b({11'b0, n }), .cin(1'b1), .sumout(out9), .cout(cout9));
	
//nextTopIdx + n	
	wire [17:0] out10;
	wire cout10;
	adder_18bit u_add10(.a({6'b0, nextTopIdx}), .b({11'b0, n }), .cin(1'b0), .sumout(out10), .cout(cout10));

//nextTopIdx + 1	
	wire [17:0] out11;
	wire cout11;
	adder_18bit u_add11(.a({6'b0, nextTopIdx}), .b(18'b0), .cin(1'b1), .sumout(out11), .cout(cout11));

//curTopIdx+1	
	wire [17:0] out12;
	wire cout12;
	adder_18bit u_add12(.a({6'b0, curTopIdx}), .b(18'b0), .cin(1'b1), .sumout(out12), .cout(cout12));
...
begin
	...
	begin
		if ((imodk == 32-1 && mode == 2'b00) || (i1modk == 32-1 && mode == 2'b01))
			diagIdx <= diagIdx + 2 + 1; //ignore
		else
			diagIdx <= diagIdx + 2; //ignore
	end
	...
	begin
		if (i1modk == 32-1 && mode[0] == 1'b0)
			leftIdx <= leftIdx + 2 + 1; //ignore
		else
			leftIdx <= leftIdx + 2; //ignore
	end
	...
	if (start == 1'b1)
		...
		if (mode[1] == 1'b0)
			msIdx <= leftIdx + 2; //ignore
		else
			msIdx <= topIdx;
	else if (nextRowState == `cLOAD_ROW_INC_J)
		msIdx <= msIdx + 2; //ignore
	...
	if (start == 1'b1)
		...
		if (mode[1] == 0)
			//nextTopIdx <= nextTopIdx + n + 1; //12 bit + 7 bit + 1 bit
			nextTopIdx <= out9[11:0];
		else
			//nextTopIdx <= nextTopIdx + n; //12 bit + 7 bit
			nextTopIdx <= out10[11:0];
			//nextTopIdx2 <= nextTopIdx + n + 1; //12 bit + 7 bit + 1 bit
			nextTopIdx2 <= out9[11:0];

	if (start == 1'b1)
		curTopIdx <= 12'b000000000001;
	else if (currentState == `cUPDATE_J)
   if (mode[1] == 1'b0)
		  //curTopIdx <= nextTopIdx+1; //12 bit + 1 bit
		  curTopIdx <= out11[11:0];
   else
		  curTopIdx <= nextTopIdx;
	else if (nextRowState == `cLOAD_ROW_INC_J)
		//curTopIdx <= curTopIdx + 1; //12 bit + 1 bit
		curTopIdx <= out12[11:0];
...
// compute cycles of delay in FSM
	if (currentState == `cSTORE_MO)
		waitCycles <= 32-1; //ignore
	else if (currentState == `cINCRE_I)
	begin
		if (i1 == stop-1)
			if (mode[1] == 1'b1)
				waitCycles <= 32-1 + 6 - 3; //ignore
			else
				waitCycles <= waitCycles + 5 - 2;  //ignore 
		else if (mode == 2'b01 && waitCycles < 32-1 - (16-1) - 4)  //ignore
			waitCycles <= 32-1 - (16-1) - 4;  //ignore
		else if (mode == 2'b10 && i1modk == 32-1)  //ignore
			waitCycles <= 32-1 + 6 - 3;  //ignore
		else if (mode == 2'b00)
			waitCycles <= waitCycles + 6 ;  //ignore
	end
else if (waitCycles >7'b0000000)
		waitCycles <= waitCycles - 1; //ignore
...
begin
	if (currentRowState == `cDONE_FETCH_ROW)
		...
		startFetchRow = 0;
	if (currentState == `cMULT_SUB && topIdxCounter+2 == mdivk)  //ignore
		loadRow = 1;
	else
		loadRow = 0;
...
begin
	...
	`cDONE_FETCH_ROW:
		if (startFetchRow == 1)
			nextRowState = `cFETCH_ROW;
		else if (loadRow == 1 || (topIdx+1 == mdivk && nextState == `cMULT_SUB))  //ignore
			nextRowState = `cLOAD_ROW_INC_J;
		else
			nextRowState = `cDONE_FETCH_ROW;
	`cLOAD_ROW_INC_J:
		if (topIdx+1 == mdivk && nextState == `cMULT_SUB)  //ignore
			nextRowState = `cLOAD_ROW_INC_J;
		else
			nextRowState = `cDONE_FETCH_ROW;
	default:
	...
	wire [17:0] out13;
	wire cout13;
	adder_18bit u_add13(.a({6'b0, nextTopIdx}), .b({11'b0, n }), .cin(1'b1), .sumout(out13), .cout(cout13));
	
	wire [17:0] out14;
	wire cout14;
	adder_18bit u_add14(.a({6'b0, nextTopIdx}), .b({11'b0, n }), .cin(1'b0), .sumout(out14), .cout(cout14));
	
	wire [17:0] out15;
	wire cout15;
	adder_18bit u_add15(.a({6'b0, nextTopIdx}), .b(18'b0), .cin(1'b1), .sumout(out15), .cout(cout15));
/* 	wire [17:0] out13;
	wire cout13;
	wire [17:0] out14;
	wire cout14;
...
begin
	if (updateCounter == 1 || currentRowState == `cLOAD_ROW_INC_J)
		...
		topIdxCounter <= topIdxCounter + 1;  //ignore

	if (updateCounter == 1)
		diagIdxCounter <= diagIdx;
	else
		diagIdxCounter <= diagIdxCounter + 1;  //ignore

	if (updateCounter == 1 || currentRowState == `cLOAD_ROW_INC_J)
		msIdxCounter <= msIdx;
	else
		msIdxCounter <= msIdxCounter + 1;  //ignore

	if (updateCounter == 1 || currentRowState == `cLOAD_ROW_INC_J)
		leftIdxCounter <= leftIdx;
	else
		leftIdxCounter <= leftIdxCounter + 1;  //ignore
	
	if (currentState == `cFETCH_COL || currentState == `cSTORE_MO)
		topWriteCounter <= i1;
	else if (writeRow == 1 || currentRowState == `cFETCH_ROW)
		topWriteCounter <= topWriteCounter + 1;  //ignore

	if (currentState == `cSTART)
		nextTopIdxCounter <= nextTopIdx;
 else if (currentState == `cSTORE_MO)
		if (mode[1] == 0)
			//nextTopIdxCounter <= nextTopIdx + n + 1; // 12 bit + 7 bit + 1 bit
			nextTopIdxCounter <= out13[11:0];
		else
			//nextTopIdxCounter <= nextTopIdx + n; //12 bit + 7 bit 
			nextTopIdxCounter <= out14[11:0];
			
	else if (writeRow == 1 || currentRowState == `cFETCH_ROW)
		//nextTopIdxCounter <= nextTopIdxCounter + 1;
		nextTopIdxCounter <= out15[11:0];
	if (currentState == `cSTART)
			readRowCounter <= 0; //offsetdivk;
	else if (currentState == `cSTORE_MO)
		if (mode[1] == 0)
			readRowCounter <= leftIdx + 2;  //ignore
		else
			readRowCounter <= topIdx;
	else if (writeRow == 1 || currentRowState == `cFETCH_ROW)
	   readRowCounter <= readRowCounter + 2;  //ignore

	if (updateCounter == 1)
		counter <= 0;
	else
		counter <= counter + 1;  //ignore

	if (currentState == `cSTORE_DIAG || currentState == `cSTORE_DIAG2)
		divCounter <= 0;
	else if (divCounter < 56)
		divCounter <= divCounter + 1;  //ignore
...
wire [31:0] out16;
wire cout16;
adder_32bit u_add16(.a({8'b0, mem_addr5}), .b({29'b0, `BURSTLEN}), .cin(1'b0), .sumout(out16), .cout(cout16));
...
begin
	...
	begin
		data_count <= data_count - `BURSTLEN; // ignore
		
		//mem_addr5 <= mem_addr5 + `BURSTLEN; //24 bit + 3 bit 
		mem_addr5 <= out16[23:0];
	...
	begin
		size <= size - 1;
		data_count <= data_count + `RATIO - `BURSTLEN;  //ignore
		//mem_addr5 <= mem_addr5 + `BURSTLEN; //24 bit + 3 bit 
		mem_addr5 <= out16[23:0];
		ram_addr4 <= ram_addr4+1; //7 bit + 1 bit 
		//ram_addr4 <= out17[23:0];
...
begin // : STATUS_COUNTER
	if ((rdreq) && (!wrreq) && (status_cnt != 0))
		status_cnt <= status_cnt - 1'b1;  //ignore
// Write but no read.
...
assign junk_input = 1024'b0;
 ...
 begin  //WRITE_POINTER
	if (wrreq) 
	begin
 		wr_pointer <= wr_pointer + 1'b1;  //ignore
	end
end
always @ (posedge clk)
begin  //READ_POINTER
	if (rdreq) 
	begin
	rd_pointer <= rd_pointer + 2'b01;  //ignore
	end
end
always  @ (posedge clk )
begin  //READ_DATA
...
if (rdreq) 
begin
rd_pointer <= rd_pointer + 1'b1;  //ignore
end
end
always  @ (posedge clk )
begin  //READ_DATA
...
begin // : STATUS_COUNTER
	...
	// Write but no read.
		...
		status_cnt <= status_cnt + 1;  //ignore
end
  dual_port_ram ram_addr(
.we1      (wrreq)      , // write enable
 .we2      (rdreq)       , // Read enable
...
wire [31:0] out18;
wire cout18;
adder_32bit u_add18(.a({8'b0, a_man}), .b({8'b0, b_man}), .cin(1'b0), .sumout(out18), .cout(cout18));
...
assign cout18 = 1'b0; */
	...
	always @ (a_man or b_man or a or b) begin  
		...
		//both positive  
		if (~a[31] && ~b[31]) begin  
			//sum_man = a_man + b_man;   // 24 bit + 24 bit 
			sum_man = out18[24:0];
			sum[31] = 1'b0;  
		end   
		...
		//a pos, b neg  
			...
			if (smaller) begin //a < b  
				//sum_man = b_man - a_man;   // 24 bit + 24 bit 
				sum_man = out18[24:0];
				sum[31] = 1'b1;  
			end else begin  
				//sum_man = a_man - b_man;   // 24 bit + 24 bit 
	...
	always @ (sum_man or a_exp) begin  
		if (sum_man[24])begin //shif sum >> by 1, add 1 to the exponent.  
			...
			sum[30:23] = a_exp + 8'b00000001;  //ignore
...
//div = result  
	...
	always @ (n_exp or d_exp) begin  
		if (n_exp >= d_exp) begin  
			div_exp = 8'b01111111 + (n_exp - d_exp);  //ignore
		end else begin  
			div_exp = 8'b01111111 - (d_exp - n_exp);  //ignore
		end  
	end  
...
module exponent(expa, expb, expsum, twoormore, tiny); 
  ...
  //assign expsum = expa + expb - `BIAS + twoormore; //10 bit + 10 bit + 7 bit + 1 bit
  	wire [17:0] out20;
	wire cout20;
	adder_18bit u_add20(.a({8'b0, expa}), .b({8'b0, expb}), .cin(twoormore), .sumout(out20), .cout(cout20));
...
//  assign roundzero	= &roundmode || (^roundmode && (roundmode[0] || sign)); 
  ...
  assign tempexp 	= roundoverflow + shiftexp;  // 1 bit + 10 bit 
